...
const vm = {
  ...
  methods: {
    ...
    loadFromAPI: function() {
      ...
      axios
        .get(
          `${API_URL}/${date[2]}-${date[1]}-${date[0]}/${this
            .workspaceId}/${this.token}`
        )
        .then(response => {
          ...
          tasks.forEach(task => {
            const resultProjects = tasks.filter(
              taskF => taskF.project === task.project
            );

            if (projs.indexOf(task.project) === -1 && task.project) {
              resProjects.push({
                project: task.project,
                tasks: resultProjects,
              });
            }
          ...
          this.tasks = resProjects.map(project => {
            ...
            project.tasks.forEach(task => {
              ...
              if (tags.indexOf(task.tags) === -1) {
                rmTasks = project.tasks.filter(
                  taskF => taskF.tags === task.tags
                );
            ...
            return {
              ...
              tasks: tasks,
              formattedTime: this.formatTime(totalTimeProject * 3600),
            };
          });
        })
        .catch(error => {
